...
# GENERATE_SEED=42
GENERATE_SEED=40
def load_cifar10(ds, batch_size, device):
    test_transform = transforms.Compose(
            [transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.247, 0.243, 0.261))])
    
    temp_train = dsets.CIFAR10(root='data/', train=True, download=False, transform=transforms.ToTensor())
    temp_valid = dsets.CIFAR10(root='data/', train=True, transform=test_transform)
    ...
    partial_matrix_dataset = CIFAR10_Augmentention(traindata, partialY.float(), trainlabels.float())
